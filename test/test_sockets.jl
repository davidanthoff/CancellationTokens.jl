...
@testitem "readline(TCPSocket) - data arrived, cancel after completion returns data" begin
    import Sockets
    ...
    @async begin
        conn = Sockets.accept(server)
        println(conn, "hello")
        # Keep connection open so readline doesn't see EOF
        sleep(5.0)
        close(conn)
    end
