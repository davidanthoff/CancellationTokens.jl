# The cancellable-eof stream support is only available on Julia ≥ 1.6 (see
# src/streams.jl), so these test items are skipped on older versions.

@testitem "eof(stream) - cancel is non-destructive" setup=[SpawnHelper] skip=(VERSION < v"1.6") begin
    pipe = Pipe()
...
@testitem "eof(stream) - data already available" skip=(VERSION < v"1.6") begin
    pipe = Pipe()
    Base.link_pipe!(pipe; reader_supports_async=true, writer_supports_async=true)
    write(pipe, "x")
...
@testitem "eof(stream) - blocks until data arrives" setup=[SpawnHelper] skip=(VERSION < v"1.6") begin
    pipe = Pipe()
    Base.link_pipe!(pipe; reader_supports_async=true, writer_supports_async=true)
...
@testitem "eof(stream) - end of stream" setup=[SpawnHelper] skip=(VERSION < v"1.6") begin
    pipe = Pipe()
    Base.link_pipe!(pipe; reader_supports_async=true, writer_supports_async=true)
...
@testitem "eof(stream) - concurrent Base reader is unaffected" setup=[SpawnHelper] skip=(VERSION < v"1.6") begin
